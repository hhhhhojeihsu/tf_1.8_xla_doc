...
/**
 * \brief Updates the resource with values from argument `pack`.
 *
 * 1. Based on the argument `gradient_sources`:
 *  - If empty: treat `pack` as a single element.
 *  - If non-empty: treat `pack` as a tuple that represents a TensorArray and its gradients, and unpacks and updates the gradient resources stored in the resource.
 */
