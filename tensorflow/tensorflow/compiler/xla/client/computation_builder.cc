 ...
 * Unclear parts:
 * 
 * 1. Check on line [65](https://hhhhhojeihsu.github.io/tensorflow_1.8_woboq/tensorflow_1.8_xla/tensorflow/tensorflow/compiler/xla/client/computation_builder.cc.html#65)
 */
...
/**
 * \brief Run the given parameter `op_request` and fill in `op_response`.
...
/**
 * \brief Call `RunOp()` and either return the output `ComputationDataHandle` (on success) or an empty `ComputationDataHandle` (on failure).
...
/**
 * \brief Enqueues a "retrieve parameter value" instruction to the `UserComputation`.
