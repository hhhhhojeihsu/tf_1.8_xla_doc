...
class IrEmitter : public DfsHloVisitorWithDefault {
 public:
	/**
   * \brief Create a new LLVM IR emitter.
   *
   * \param hlo_module The HLO module we are emitting IR for.
   * \param assignment A BufferAssignment from which we know which temporary buffers
   *             are used by the HLO nodes.
   * \param llvm_module The LLVM module to emit IR into.
   * \param instruction_to_profile_idx The mapping from HLO instructions to their
   *              index in the profiling array.
   * \param computation_to_profile_idx The mapping from HLO computations to their
   *              index in the profiling array.
   * \param external_constant_pool If non-null, points to an ExternalConstantPool
   ...
   * function.
   *
   * \param function_name_prefix The desired name of the function. If the name is
   * not unique among already emitted functions then a suffix is appended to
   * make the name unique.
   *
   * \param is_top_level_computation It has the following meanings for each CPU backend:
   ...
   *
   * \param instruction_order If not NULL, then the HLO instructions are emitted
