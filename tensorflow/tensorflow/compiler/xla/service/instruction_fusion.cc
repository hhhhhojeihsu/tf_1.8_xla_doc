...
/**
 * \brief Check whether `producer` can fuse into `consumer` along each path between them.
 * * Build `could_fuse_on_all_paths` lambda, which basically perform all the check.
 *   * Check whether `producer` is already in `do_not_fuse`.
 *   * Check whether `producer` or `consumer` is not fusable.
 *   * For each operand of `consumer`, `consumer_operand`
 *     * If `consumer_operand == producer`, we reach the base case. Call `ShouldFuse` to check whether they shouldn't be fused.
 *     * Else if `producer` is reachable from `consumer_operand`
 *       * Check whether `consumer_operand` is already in `do_not_fuse`.
 *       * Check whether `consumer_operand` is not fusable.
 *       * Check whether `consumer` and `consumer_operand` shouldn't be fused.
 *       * Call `CanFuseOnAllPaths` recursively, with `producer` as `producer`, `consumer_operand` as `consumer`.
 * * Call `could_fuse_on_all_paths`, insert `producer` to `do_not_fuse` if any of aforementioned check holds.
 */
...
/**
 * * For each non-fusion computation `computation` in `module`
 *   * Assign `computation` to `computation_`, meaning current `HloComputation` instance the loop fuser is traversing.
 *   * Build `post_order` vector and `post_order_index` map to maintain information of post-order traversal.
 *   * Build `reachability`, recording from each consumer, whether a certain producer is reachable.
 *   * Build a lambda `cheap_to_duplicate`.
 *   * Populate `do_not_fuse`, record instructions which won't server as producer.
 *   * Iterate `post_order` reversely until it becomes empty
 *     * Fetch producer from `post_order` and record as `instruction`.
 *     * If `instruction == NULL`, meaning it has been removed during fusion, just `continue`.
 *     * Remove entry of `instruction` in `post_order_index`.
 *     * If `instruction` isn't fusable and its opcode isn't `kFusion`, just `continue`.
 *     * Sort operands' index of `instruction` in reverse post-order(RPO) and store in `sorted_operand_numbers`. This is to avoid needless duplication. Look up the elaborated [comment](https://github.com/tensorflow/tensorflow/blob/r1.8/tensorflow/compiler/xla/service/instruction_fusion.cc#L306).
 *     * For each `operand` indexed by `sorted_operand_numbers`
 *       * Perform following checks, `continue` if any of them fails.
 *       * `fusion_instruction = Fuse(operand, instruction)`
 *       * Push `fusion_instruction` to end of `post_order`(this way it will be fetched next in RPO), and update `post_order_index`.
 *       * If there is no users of `operand`, remove it.
 */
...
/**
 * * Choose a fusion kind.
 * * Check whether `consumer` is `kFusion` instruction
 *   * If so, assign `fusion_instruction` as `consumer`, and update fusion kind of it.
 *   * Else, Create a fusion instruction(along with the corresponding fusion computation), then calling `ReplaceInstruction(consumer, fusion_instruction)`.
 * * Calling `fusion_instruction->FuseInstruction(producer)` to fuse `producer`.
 */
