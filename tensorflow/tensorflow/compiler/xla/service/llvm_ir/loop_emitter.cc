...
/**
 * * `body_emitter_` call given `target_element_generatortarget` to emit code to calculate the result at the given index, then emit `store` instruction to write the result to `target_array`.
 * * `body_emitter_` will be invoked in `EmitLoop()` after the nested loop is emitted.
 */
...
/**
 * * Generate a nested for-loop based on the shape of target array.
 * * Invoke the `body_emitter_` to populate the content of innermost loop.
 */
